...
# General wrapper function for calculating metrics (for backward compatibility)
def calculate_metrics(y_true: np.ndarray, y_pred: np.ndarray, task_type: str = 'classification',
                     threshold: float = 0.5) -> Dict[str, float]:
    """
    ...
    Args:
        y_true: Ground truth labels
        y_pred: Predicted values (probabilities for classification, continuous values for regression)
        task_type: Type of task ('classification' or 'regression')
        threshold: Threshold for converting probabilities to binary predictions (for classification only)
...
def calculate_regression_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    ...
    metrics['r2'] = 1 - (ss_residual / ss_total) if ss_total > 0 else 0.0
...
def generate_classification_report(y_true: np.ndarray, y_pred: np.ndarray, 
    ...
    report = classification_report(y_true, y_pred_binary, target_names=['No Migraine', 'Migraine'])
    
    # Calculate additional metrics
    metrics = calculate_classification_metrics(y_true, y_pred_proba, threshold)
    
    # Format additional metrics
    additional_metrics = "\nAdditional Metrics:\n"
    additional_metrics += f"ROC AUC: {metrics['roc_auc']:.4f}\n"
    
    # Combine reports
    return report + additional_metrics


# Function to calculate expert contribution metrics (for backward compatibility)
def calculate_expert_contribution_metrics(model: torch.nn.Module, data_loader: torch.utils.data.DataLoader,
                                        device: torch.device, output_dir: Optional[str] = None) -> Dict[str, float]:
    """
    Calculate expert contribution metrics.
    ...
    Returns:
        Dictionary mapping expert names to contribution metrics
    """
    # Create metrics calculator
    metrics_calculator = MigrainePerformanceMetrics(model, device)
    
    # Calculate expert contributions
    return metrics_calculator.calculate_expert_contributions(data_loader, output_dir)
