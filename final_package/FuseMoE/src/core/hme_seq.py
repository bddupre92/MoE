...
class SparseDispatcher(object):
    ...
    def __init__(self, num_experts, gates):
        ...
        print(f"DEBUG SparseDispatcher: gates shape: {gates.shape}, num_experts: {num_experts}")
        
        # Use the simpler implementation from sparse_moe.py
        ...
        print(f"DEBUG SparseDispatcher: self._expert_index shape: {self._expert_index.shape}, values: {self._expert_index}")
        print(f"DEBUG SparseDispatcher: self._batch_index shape: {self._batch_index.shape}, values: {self._batch_index}")
        print(f"DEBUG SparseDispatcher: self._part_sizes: {self._part_sizes}")
        
        # expand gates to match with self._batch_index
        gates_exp = gates[self._batch_index.flatten()]
        print(f"DEBUG SparseDispatcher: gates_exp shape: {gates_exp.shape}")
        
        # Use simpler gather operation from sparse_moe.py
...
class HierarchicalMoE(nn.Module):
    ...
    def __init__(self, config: MoEConfig):
        ...
        # Initialize gate and noise parameters as persistent attributes
        self.w_gate = nn.ParameterList([
            nn.Parameter(torch.zeros(self.input_size, self.num_experts[level]), requires_grad=True)
            for level in range(len(self.num_experts))
        ])
    ...
    def _get_logits(self, x, train, level, noise_epsilon):
        # Use persistent class attributes initialized in __init__
        w_gate = self.w_gate[level]
        w_noise = self.w_noise[level]

        # Initialize clean_logits to handle potential unassignment
        clean_logits = None 
        ...
        if self.gating[level] == 'softmax':
            ...
            # Raise error for unsupported gating type
            raise ValueError(f"Unsupported gating type: {self.gating[level]} at level {level}")
