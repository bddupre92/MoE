...
class MigraineGating(nn.Module):
    ...
    def __init__(self, input_dims: List[int], hidden_dim: int = 64, num_experts: int = 4,
    ...
    def forward(self, inputs: List[torch.Tensor], training: bool = True) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if training:
            ...
            target_fraction = torch.ones_like(routing_fraction) / self.num_experts
            
            # Calculate the load balancing loss (coefficient of variation)
            load_balancing_loss = torch.sum((routing_fraction - target_fraction)**2) * self.load_balance_coef
        else:
            load_balancing_loss = torch.tensor(0.0, device=gates.device)
...
def create_migraine_gating(input_dims: List[int], hidden_dim: int = 64, num_experts: int = 4,
                          top_k: int = 2, dropout_rate: float = 0.2, noisy_gating: bool = True,
                          load_balance_coef: float = 0.01, learning_rate: float = 0.001) -> Tuple[MigraineGating, Dict[str, Any]]:
    """
    Create a migraine gating network with optimizer.
