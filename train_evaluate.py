...
class MigraineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # The features dictionary is returned as is
...
def evaluate_model(model, test_loader):
    ...
    try:
        ...
        roc_auc = float("nan") # Handle cases where only one class is present
...
if __name__ == "__main__":
    ...
    train_inputs, train_targets = adapter.adapt_data_for_fusemoe(X_train_df, y_train_series, physio_dim_expected=model.physio_dim)
    valid_inputs, valid_targets = adapter.adapt_data_for_fusemoe(X_valid_df, y_valid_series, physio_dim_expected=model.physio_dim)
    test_inputs, test_targets = adapter.adapt_data_for_fusemoe(X_test_df, y_test_series, physio_dim_expected=model.physio_dim)
    
    if train_inputs is None or valid_inputs is None or test_inputs is None:
        print("Error adapting data. Exiting.")
        sys.exit(1)

    train_dataset = MigraineDataset(train_inputs, train_targets)
    valid_dataset = MigraineDataset(valid_inputs, valid_targets)
    test_dataset = MigraineDataset(test_inputs, test_targets)
