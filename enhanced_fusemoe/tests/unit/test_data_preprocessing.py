...
class TestSyntheticDataGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.generator = MigraineSyntheticDataGenerator(
            num_samples=100,
            time_periods=7,
            seed=42
        )
...
class TestDataPreprocessor(unittest.TestCase):
    ...
    def test_create_data_loaders(self):
        ...
        # Check batch size - get first batch and verify structure
        for inputs, targets in train_loader:
            # Verify inputs is a dictionary with expert names as keys
            self.assertIsInstance(inputs, dict)
            # Verify each expert's data has the right shape
            for expert_name in inputs:
                self.assertLessEqual(inputs[expert_name].shape[0], 16)  # Batch size
            break
